...
x<-df_main %>% filter(e1==1) %>%  #filter results only from endline survey1
  ...
  dplyr::select(admin_cost_us,group_size,grantsize_pp_US_est3,group_existed,
                ...
                loan_100k,loan_1mil,
                assigned,S_K,S_H,S_P_m,aggression_n,
...
for(i in 1:n_split){
  ...
  train.mat<-model.matrix(training_hours_e~.-assigned, data=(aux_df%>%filter(assigned==1)))
  lasstreat<-cv.glmnet(x=train.mat,y=aux_df%>%filter(assigned==1)%>%.$training_hours_e,alpha=0)  
  
  train.mat<-model.matrix(training_hours_e~.-assigned, data=(aux_df%>%filter(assigned==0)))
  lassbase<-cv.glmnet(x=train.mat,y=aux_df%>%filter(assigned==0)%>%.$training_hours_e,alpha=0) 
  
  # predict baseline and treatment outcomes on main sample
  pred.mat<-model.matrix(training_hours_e~.-assigned, data=main_df)
  ...
  if (is.na(blp$coefficients[2])){
    blp_coef[i,]<-c(blp$coefficients[3],summary(blp)$coefficients[2:3,c(2,4)][1,],
                  blp$coefficients[4],summary(blp)$coefficients[2:3,c(2,4)][2,],
  ...
  if (is.na(blp$coefficients[2])){
  gate_diff[i,]<-summary(lm(training_hours_e~B+I(w*G),data = diff))$coefficients[2,c(1,2,4)]
  }else{
  ...
  for(k in 1:n_group){
    ...
    if (is.na(blp$coefficients[2])){
    gate_coef[i,((3*k)-2):(3*k)]<-summary(gate)$coefficients[2,c(1,2,4)]
    }else{
