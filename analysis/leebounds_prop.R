...
x<-df_main %>% filter(e1==1) %>%  #filter results only from endline survey1
  ...
  dplyr::select(admin_cost_us,group_size,grantsize_pp_US_est3,group_existed,
                ...
                aghours7da_zero,chores7da_zero,zero_hours,nonag_dummy,inschool,
                skilledtrade,education,literate,
                ...
                groupid,partid,ind_found_e1)

#eliminate NA values 
df<-x[which(complete.cases(x)),]

out<-matchit(assigned~.-groupid-partid-ind_found_e1,data=df,method='optimal',distance = 'glm',
             link='probit')

df2<-match.data(out)

y<-df_main %>% filter(e1==1) %>%#filter results only from endline survey1
  dplyr::select(partid,bizasset_val_real_p99_e)

df3<-left_join(df2,y,by='partid') %>% select(-partid,-groupid,-weights,-subclass)

x<-df3 %>%  #filter results only from endline survey1
  dplyr::select(assigned,ind_found_e1,bizasset_val_real_p99_e)

# attrited treatment
qt<-x%>%filter(assigned==1)%>%.$ind_found_e1%>%mean()

# attrited control
qc<-x%>%filter(assigned==0)%>%.$ind_found_e1%>%mean()
q<-(qt-qc)/qt
...
ub<-x2 %>% filter(assigned==1) %>%
  ...
  .[-(1:abs(floor(nrow(x2%>%filter(assigned==1))*q))),] %>% 
...
lb<-x2 %>% filter(assigned==0) %>%
  arrange(desc(bizasset_val_real_p99_e)) %>%
  .[-(1:abs(floor(nrow(x2%>%filter(assigned==0))*q))),] %>% 
...
for(i in 1:n_split){
  ...
  # endline 1: 1,1
  # endline 2: 2,6
  rftreat<-randomForest(bizasset_val_real_p99_e~.-assigned, data = (aux_df%>%filter(assigned==1)),
                        ntree=1000,nodesize=7, mtry=3)  
  rfbase<-randomForest(bizasset_val_real_p99_e~.-assigned, data = (aux_df%>%filter(assigned==0)), 
                       ntree=1000,nodesize=7, mtry=9)
