...
# impute SE
ctrl<-df3 %>% filter(assigned==0 & ind_found_e1==1)
ub<-mean(ctrl$bizasset_val_real_p99_e,na.rm=T)+
  (0.5*(sd(ctrl$bizasset_val_real_p99_e,na.rm = T)))

tre<-df3 %>% filter(assigned==0 & ind_found_e1==1)
lb<-mean(tre$bizasset_val_real_p99_e,na.rm=T)-
  (0.5*(sd(tre$bizasset_val_real_p99_e,na.rm = T)))
