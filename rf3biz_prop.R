...
ggplot(ci, aes(x = group, y = estimate)) +
  ...
  labs(
    title = "GATES with confidence bands",
  ...
  geom_hline(
    data = data.frame(yintercept = 461.753+c(-1,1)*(2.32828*94.617)), # for endline 2 it's 18.19 (4.898)
    # for endline 1 it's 14.61 (4.073)
    aes(yintercept = yintercept, color = "darkred"), linetype = "longdash"
  ) + # ATE and CI from original paper
  geom_hline(
    data = data.frame(yintercept = 461.753),
